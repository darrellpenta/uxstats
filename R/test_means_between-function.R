...
#'
#'
#' @param .x A single numeric value (i.e., the mean from sample 1) or a data frame with named columns of numeric data corresponding to the variables being tested. See Details.
...
#' @importFrom dplyr group_by group_modify summarise row_number
...
#' @export
#'
test_means_between.data.frame <-
  function(.x, .var1, .var2, ..., .alpha = 0.05) {
if (.alpha < 0 | .alpha > 1) {
  ...
  .out <-
    dplyr::group_modify(.out,
                        ~ test_means_between.numeric(.x$samp1_mean,
                                                ...
                                                .alpha = .alpha),
                        .keep = TRUE) %>%
    dplyr::select("{{.var1}}_mean" := .data$samp1_mean,
           "{{.var2}}_mean" := .data$samp2_mean,
           "{{.var1}}_stdev" := .data$samp1_stdev,
           "{{.var2}}_stdev" := .data$samp2_stdev,
           "{{.var1}}_size" := .data$samp1_size,
           "{{.var2}}_size" := .data$samp2_size,
           .data$t_val, .data$t_crit,.data$deg_fr, .data$p_val, .data$ci_low, .data$ci_hi, .data$ci_note
           )
