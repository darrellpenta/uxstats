...
#'
#'
#' @param .x A single numeric value or a long-format data frame with named columns of numeric data corresponding to the variables being tested. See Details.
...
#' @rdname two_mean_test


#' @export
#'
#'
two_mean_test <- function(.x, ...) {
  UseMethod("two_mean_test", .x)
}
#' @rdname two_mean_test
#' @export
ab_test <- two_mean_test
#'
#' @rdname two_mean_test
...
#' @export
#'
two_mean_test.numeric <- function(.x, .sd, .n, ..., .alpha = .05) {
  ...
  if(length(.x)== 1){
    .t_val<-within_t(.x,.sd,.n)
    .p_val<-within_t_to_p(.t_val,.df = (.n-1),.tails=2)
...
within_t <- function(.m,.sd,.n){

 .m/(.sd/sqrt(.n))
}

within_t_to_p<-function(.t,.df, .tails = c(2,1)){
  stats::pt(-abs(.t), df=.df) * .tails
}
...
#' @export
#'
two_mean_test.data.frame <- function(.x, .var1, .var2, ..., .alpha = 0.05) {
if (.alpha < 0 | .alpha > 1) {
  ...
  .out <-
    dplyr::group_modify(.out,
                        ~ two_mean_test.numeric(.x$diff_mean,
                                                .x$diff_stdev,
                                                .n = .x$n,
                                                .alpha = .alpha),
                        .keep = TRUE)
