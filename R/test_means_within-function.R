#' Test two means against each other for a "within-subjects" design
#'
#' @description
#' \code{test_means_within()} compares two sample means obtained from the same group of users and performs a two-sided statistical test to determine if the means differ from each other.
#'
#' \code{test_means_within}, \code{ab_test_within}, and \code{test_times_within} are synonyms.
#'
#' @details
#' \itemize{
...
#' @rdname test_means_within


#' @export
#'
#'
test_means_within <- function(.x, ...) {
  UseMethod("test_means_within", .x)
}
#' @rdname test_means_within
#' @export
ab_test_within <- test_means_within
#'
#' @rdname test_means_within
...
#' @export
#'
test_means_within.numeric <- function(.x, .sd, .n, ..., .alpha = .05) {
...
#' @export
#'
test_means_within.data.frame <- function(.x, .var1, .var2, ..., .alpha = 0.05) {
if (.alpha < 0 | .alpha > 1) {
  ...
  .out <-
    dplyr::group_modify(.out,
                        ~ test_means_within.numeric(.x$diff_mean,
                                                .x$diff_stdev,
                                                .n = .x$n,
                                                .alpha = .alpha),
                        .keep = TRUE)
